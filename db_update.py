...
# Function to delete data for yesterday
def delete_yesterday_data():
    yesterday = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%d")
    db.collection("meals").document(yesterday).delete()
    print(f"Deleted data for {yesterday}.")
...
def update_db():
    ...
    print(f"Fetching meals for {tomorrow}...")

    # Parse tomorrow's meals (use your scraping logic here)
    items = parse_day(tomorrow)  # You can reuse the `parse_day` function for tomorrow

    # Write tomorrow's meals to the database
    write_meals(tomorrow, items)

    # Clean up by deleting yesterday's meals
